...
-(instancetype)init{
    ...
    if (self) {
        ...
        {
            self.centerLightTv = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, 79, 76)];
            self.centerLightTv.image = [UIImage imageNamed:[@"WMPlayer.bundle" stringByAppendingPathComponent:@"play_new_brightness_day"]];
            self.centerLightTv.center = CGPointMake(155*0.5, 155*0.5);
            [self addSubview:self.centerLightTv];
        }
...
-(void)layoutSubviews{
    ...
    self.transform = [WMPlayer getCurrentDeviceOrigination];
